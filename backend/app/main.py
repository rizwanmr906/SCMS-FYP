...
def ensure_database() -> None:
    ...
    cursor.execute(
        ...
        CREATE TABLE IF NOT EXISTS complaints (
            ...
            voice_note_data BLOB,
            voice_note_mime_type TEXT,
            FOREIGN KEY (user_id) REFERENCES users(id),
            FOREIGN KEY (department_id) REFERENCES departments(id)
        )
        """
    ...
    if "voice_note_data" not in complaint_columns:
        cursor.execute("ALTER TABLE complaints ADD COLUMN voice_note_data BLOB")
    if "voice_note_mime_type" not in complaint_columns:
        cursor.execute("ALTER TABLE complaints ADD COLUMN voice_note_mime_type TEXT")
    cursor.execute(
        """
...
def get_voice_note(complaint_id: int, current_user: Dict[str, Any] = Depends(get_current_user)):
    conn = db_connect(); cursor = conn.cursor()
    cursor.execute("SELECT user_id, department_id, voice_note_data, voice_note_mime_type FROM complaints WHERE id = ?", (complaint_id,))
    row = cursor.fetchone(); conn.close()
